...
LRESULT CALLBACK setRadiusDialog(HWND hdlg, UINT mess, WPARAM more, LPARAM pos); //function definition for taking inputs from the dialogue box.
...
void processCommonCommand(int cmd, HWND hwnd)
{
	...
	case ID_CLEAR:
		...
		break;
	case ID_SET_RADII:
		DialogBox(NULL, "MyDB", hwnd, (DLGPROC)setRadiusDialog);
		setDrawMode(DRAW_MODE, hwnd);
		break;
...
LRESULT CALLBACK setRadiusDialog(HWND hdlg, UINT mess, WPARAM more, LPARAM pos)
{
	char str[20];
	switch (mess)
	{
